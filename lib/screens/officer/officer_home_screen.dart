  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: _selectedNavIndex == 0
          ? _buildFeedBody()
          : _selectedNavIndex == 1
              ? _buildFeedBody() // Reports tab
              : _selectedNavIndex == 2
                  ? const MapScreen() // Live Map/Alerts tab
        ...
        Expanded(
          child: StreamBuilder<QuerySnapshot>(
            ...
            builder: (context, snapshot) {
              ...
              // Filter documents based on search & filter tabs
              ...
              return ListView.builder(
                padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
                itemCount: filteredDocs.isEmpty ? 4 : filteredDocs.length + 1, // Fallback mock cards if DB empty
                itemBuilder: (context, index) {
                  // Show mock demo cards from design if database has few items
                  if (filteredDocs.isEmpty) {
                    return _buildDemoCard(index);
                  }

                  if (index == 1) {
                    // Embed Proximity Map Preview (Matching Image 5)
            ...
            children: [
              ...
              GestureDetector(
                onTap: () => Navigator.push(
                  context,
                  MaterialPageRoute(
                    builder: (_) => CaseDetailScreen(reportId: reportId, data: data),
                  ),
                ),
                child: Row(
                  children: const [
